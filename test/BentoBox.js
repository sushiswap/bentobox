...
describe('BentoBox', function () {
  ...
  describe('Deploy', function () {
    it('Emits LogDeploy event with correct arguments', async function () {
      ...
      await expect(this.bentoBox.deploy(this.masterContractMock.address, data, true)).to.emit(
        this.bentoBox,
        'LogDeploy'
      )
    })
  ...
  describe('Conversion', function () {
    it('Should convert Shares to Amounts', async function () {
      ...
      expect(await this.bento.toShare(this.a.address, bentoProtocolLimit.toString(), false)).to.be.equal(
        bentoProtocolLimit.toString()
      )
      expect(await this.bento.toShare(this.a.address, computationalLimit.toString(), false)).to.be.equal(
        computationalLimit.toString()
      )
      expect(await this.bento.toShare(this.a.address, extremeValidVolume.toString(), true)).to.be.equal(
        extremeValidVolume.toString()
      )
      expect(await this.bento.toShare(this.a.address, bentoProtocolLimit.toString(), true)).to.be.equal(
        bentoProtocolLimit.toString()
      )
      expect(await this.bento.toShare(this.a.address, computationalLimit.toString(), true)).to.be.equal(
        computationalLimit.toString()
      )
    })
    ...
    it('Should convert amount to Shares', async function () {
      ...
      expect(await this.bento.toAmount(this.a.address, extremeValidVolume.toString(), false)).to.be.equal(
        extremeValidVolume.toString()
      )
      expect(await this.bento.toAmount(this.a.address, bentoProtocolLimit.toString(), false)).to.be.equal(
        bentoProtocolLimit.toString()
      )
      expect(await this.bento.toAmount(this.a.address, computationalLimit.toString(), false)).to.be.equal(
        computationalLimit.toString()
      )
      expect(await this.bento.toAmount(this.a.address, extremeValidVolume.toString(), true)).to.be.equal(
        extremeValidVolume.toString()
      )
      expect(await this.bento.toAmount(this.a.address, bentoProtocolLimit.toString(), true)).to.be.equal(
        bentoProtocolLimit.toString()
      )
      expect(await this.bento.toAmount(this.a.address, computationalLimit.toString(), true)).to.be.equal(
        computationalLimit.toString()
      )
    })
    ...
    it('Should convert at ratio', async function () {
      ...
      await expect(this.bento.toShare(this.a.address, computationalLimit.toString(), false)).to.be.revertedWith(
  ...
  describe('Deposit', function () {
    it('Reverts with to address zero', async function () {
      ...
      await expect(this.bentoBox.deposit(ADDRESS_ZERO, this.alice.address, ADDRESS_ZERO, 0, 0)).to.be.revertedWith(
        'BentoBox: to not set'
      )
    ...
    it('Reverts without approval', async function () {
      ...
      await expect(
        this.bentoBox.deposit(this.a.address, this.alice.address, this.bob.address, 100, 0)
      ).to.be.revertedWith('BoringERC20: TransferFrom failed')
      await expect(
      ...
      await expect(
        this.bentoBox.connect(this.bob).deposit(this.b.address, this.bob.address, this.bob.address, 100, 0)
      ).to.be.revertedWith('BoringERC20: TransferFrom failed')
    ...
    it('Mutates balanceOf correctly', async function () {
      ...
      await expect(this.bentoBox.deposit(this.c.address, this.alice.address, this.alice.address, 1, 0)).to.not.emit(
        this.c,
        'Transfer'
      )
      await expect(this.bentoBox.deposit(this.c.address, this.alice.address, this.alice.address, 999, 0)).to.not.emit(
        this.c,
        'Transfer'
      )
      await expect(this.bentoBox.deposit(this.c.address, this.alice.address, this.alice.address, 0, 1000)).to.emit(
        this.c,
        'Transfer'
      )
    ...
    it('Mutates balanceOf for BentoBox and WETH correctly', async function () {
      ...
      await expect(
        this.bentoBox
          .connect(this.bob)
          .deposit(ADDRESS_ZERO, this.bob.address, this.bob.address, 1000, 0, { value: 1000 })
      )
      ...
      expect(await this.bentoBox.balanceOf(this.weth9.address, this.bob.address), 'bob should have weth').to.be.equal(
        1000
      )
    })
    ...
    it("Reverts if TotalSupply of token is Zero or if token isn't a token", async function () {
      ...
      await expect(
        this.bentoBox
          .connect(this.bob)
          .deposit(this.bentoBox.address, this.bob.address, this.bob.address, 1, 0, { value: 1 })
      ).to.be.revertedWith(
    ...
    it('Mutates balanceOf and totalSupply for two deposits correctly', async function () {
      ...
      expect((await this.bentoBox.totals(this.a.address)).elastic, 'incorrect total amount').to.be.equal(
        getBigNumber(430)
      )
      // 130 + 100 + 200 = 430
      ...
      expect(
        ...
        'incorrect amount calculation'
      ).to.be.equal('692307692307692307688')
      expect((await this.bentoBox.totals(this.a.address)).elastic, 'incorrect total amount').to.be.equal(
        getBigNumber(1330)
      )
    })
  ...
  describe('Deposit Share', function () {
    ...
    it('should not allow grieving attack with deposit of Share', async function () {
      ...
      const ratio =
        (await this.bentoBox.totals(this.c.address)).elastic /
        (await this.bentoBox.balanceOf(this.c.address, this.alice.address))
  ...
  describe('Withdraw', function () {
    ...
    it('Reverts when attempting to withdraw below 1000 shares', async function () {
      ...
      await expect(
        this.bento.withdraw(this.a.address, this.alice.address, this.alice.address, 0, 2)
    ...
    it('Reverts when attempting to withdraw larger amount than available', async function () {
      ...
      await expect(
        this.bentoBox.withdraw(this.a.address, this.alice.address, this.alice.address, getBigNumber(2), 0)
    ...
    it('Mutates balanceOf of Token and BentoBox correctly', async function () {
      ...
      await expect(
        this.bentoBox
          .connect(this.bob)
          .deposit(this.a.address, this.bob.address, this.bob.address, getBigNumber(260), 0)
      )
        .to.emit(this.a, 'Transfer')
      ...
      expect(await this.a.balanceOf(this.alice.address), 'alice should have all of their tokens back').to.equal(
        startBal
      )
    ...
    it('Mutates balanceOf on BentoBox for WETH correctly', async function () {
      ...
      await this.bentoBox
        .connect(this.bob)
        .deposit(ADDRESS_ZERO, this.bob.address, this.bob.address, getBigNumber(1), 0, {
  ...
  describe('Withdraw From', function () {
    it('Mutates bentoBox balanceOf and token balanceOf for from and to correctly', async function () {
      ...
      expect(await this.a.balanceOf(this.bob.address), 'bob should have received their tokens').to.be.equal(
  ...
  describe('Transfer', function () {
    it('Reverts when address zero is given as to argument', async function () {
      await expect(this.bentoBox.transfer(this.a.address, this.alice.address, ADDRESS_ZERO, 1)).to.be.revertedWith(
        'BentoBox: to not set'
      )
    })
    ...
    it('Reverts when attempting to transfer amount below bento protocol limit but above balance', async function () {
      await expect(
        this.bentoBox
          .connect(this.bob)
          .transfer(this.a.address, this.bob.address, this.alice.address, extremeValidVolume.toString())
      ).to.be.revertedWith('BoringMath: Underflow')
    })
    ...
    it('Reverts when attempting to transfer bento protocol limit', async function () {
      await expect(
        this.bentoBox
          .connect(this.bob)
          .transfer(this.a.address, this.bob.address, this.alice.address, bentoProtocolLimit.toString())
      ).to.be.revertedWith('BoringMath: Underflow')
    })
    ...
    it('Reverts when attempting to transfer computational limit', async function () {
      await expect(
        this.bentoBox
          .connect(this.bob)
          .transfer(this.a.address, this.bob.address, this.alice.address, computationalLimit.toString())
      ).to.be.revertedWith('BoringMath: Underflow')
    })
    ...
    it('Mutates balanceOf for from and to correctly', async function () {
      ...
      ).to.be.equal('26923076923076923076')
      expect(
        await this.bentoBox.balanceOf(this.a.address, this.bob.address),
        'token should be transferred'
  ...
  describe('Transfer Multiple', function () {
    it('Reverts if first to argument is address zero', async function () {
      await expect(this.bentoBox.transferMultiple(this.a.address, this.alice.address, [ADDRESS_ZERO], [1])).to.be
        .reverted
    })
  ...
  describe('Skim', function () {
    it('Skims tokens to from address', async function () {
      ...
      expect(await this.bentoBox.balanceOf(this.a.address, this.bob.address), 'bob should have no tokens').to.be.equal(
        0
      )

      await expect(
        this.bentoBox.connect(this.bob).deposit(this.a.address, this.bentoBox.address, this.bob.address, 100, 0)
      )
        .to.emit(this.bentoBox, 'LogDeposit')
      ...
      await expect(
        this.bentoBox
          ...
          .deposit(
            ...
            this.bob.address,
            bigInt(bentoProtocolLimit).add(1).toString(),
            0
          )
      ).to.be.revertedWith('BentoBox: Skim too much')
  ...
  describe('Batch', function () {
    it('Batches calls with revertOnFail true', async function () {
      ...
      const transfer = this.bentoBox.interface.encodeFunctionData('transfer', [
        this.a.address,
        this.alice.address,
        this.bob.address,
        76,
  ...
  describe('set Strategy', function () {
    ...
    it('should be reverted if 2 weeks are not up', async function () {
      await this.bentoBox.setStrategy(this.a.address, this.a.address)
      await expect(this.bentoBox.setStrategy(this.a.address, this.a.address)).to.be.revertedWith(
