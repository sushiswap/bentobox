...
describe('ERC20', function () {
  ...
  describe('Transfer', function () {
    ...
    it('Succeeds for zero value transfer', async function () {
      await expect(() => this.token.transfer(this.bob.address, 0)).to.changeTokenBalances(
        this.token,
        [this.alice, this.bob],
        [-0, 0]
      )
  ...
  describe('TransferFrom', function () {
    it('transferFrom should fail if balance is too low', async function () {
      await expect(this.token.transferFrom(this.alice.address, this.bob.address, 10001)).to.be.revertedWith(
