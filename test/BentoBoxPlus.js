...
describe("BentoBoxPlus", function () {
  ...
  describe('FlashLoan', function () {
    it('should revert on batch flashloan if not enough funds are available', async function (){
      ...
      await expect(this.bentoBox.batchFlashLoan(this.flashLoaner.address, [this.flashLoaner.address], [this.a.address], [getBigNumber(1)], param)).to.be.revertedWith(
    ...
    it('should revert on flashloan if amount is not paid back', async function () {
      ...
      await this.bentoBox.deposit(this.a.address, this.alice.address, this.alice.address, getBigNumber(1), 0)
      const param = this.bentoBox.interface.encodeFunctionData("toShare", [this.a.address, 1])      
      await expect(this.bentoBox.flashLoan(this.sneakyFlashLoaner.address, this.sneakyFlashLoaner.address, this.a.address, getBigNumber(1), param)).to.be.revertedWith(
    ...
    it('should revert on batch flashloan if amount is not paid back', async function () {
      ...
      await this.bentoBox.deposit(this.a.address, this.alice.address, this.alice.address, getBigNumber(1), 0)
      const param = this.bentoBox.interface.encodeFunctionData("toShare", [this.a.address, 1])      
      await expect(this.bentoBox.batchFlashLoan(this.sneakyFlashLoaner.address, [this.sneakyFlashLoaner.address], [this.a.address], [getBigNumber(1)], param)).to.be.revertedWith(
        "BentoBoxPlus: Wrong amount")
    })
