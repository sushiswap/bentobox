...
assertBN = (a, b, string) => {
  assert.equal(a.toString(), b.toString(), string);
}
...
signERC2612Permit = async (token, owner, spender, value, deadline, nonce) => {
    ...
    const typedData = {
        ...
        primaryType: "Permit",
        domain: {
            name: await new web3.eth.Contract(pair_abi, token).methods.name().call(),
            version: '1',
            chainId: 1,
    ...
    return new Promise((resolutionFunc, rejectionFunc) => {
        web3.currentProvider.send({ method: "eth_signTypedData", params: [owner, typedData], from: owner }, function (error, result) {
            if (!error) {
