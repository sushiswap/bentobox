...
describe("MasterContractManager", function () {
  ...
  describe("Set Master Contract Approval with Permit", function () {
    ...
    it("Reverts with non whiteListed master contract", async function () {
      await expect(setMasterContractApproval(this.mcmanager, this.carol, this.carol, "", "0x0000000000000000000000000000000000000001", true, true)).to.be.revertedWith(
    ...
    it("Reverts with user not equal to sender", async function () {
      ...
      await expect(setMasterContractApproval(this.mcmanager, this.carol, this.alice, "", this.lendingPair.address, true, true)).to.be.revertedWith(
        "MasterCMgr: user not sender"
      )
    })
  ...
  describe("setMasterContractApproval with Permit", function () {
    ...
    it("Reverts if signature is incorrect", async function () {
      await expect(setMasterContractApproval(this.mcmanager, this.bob, this.bob, this.carolPrivateKey, this.lendingPair.address, true, false)).to.be.revertedWith(
        "MasterCMgr: Invalid Signature"
      )
    })
  })
})
