...
describe("CompositeOracle", function () {
  before(async function () {
    ...
    this.SimpleSLPOracle0 = await ethers.getContractFactory("SimpleSLPTWAP0Oracle")

    this.SimpleSLPOracle1 = await ethers.getContractFactory("SimpleSLPTWAP1Oracle")
  ...
  beforeEach(async function () {
    ...
    this.sushiToken = await this.ReturnFalseERC20.deploy("SUSHI", "SUSHI", e18("10000000"))
    await this.sushiToken.deployed()

    this.ethToken = await this.ReturnFalseERC20.deploy("WETH", "ETH", e18("10000000"))
    await this.ethToken.deployed()

    this.daiToken = await this.ReturnFalseERC20.deploy("DAI", "DAI", e18("10000000"))
    ...
    this.compositeOracleData = await this.compositeOracle.getDataParameter(this.oracleSushiEth.address, this.oracleDaiEth.address, this.oracleDataA, this.oracleDataB)
