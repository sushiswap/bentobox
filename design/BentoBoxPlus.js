...
class BentoBox {
  ...
  toAmount(token, share) {
    return this.totalShare[token] == 0 ? share : (share * this.totalAmount[token]) / this.totalShare[token]
  }

  toShare(token, amount) {
    return this.totalAmount[token] == 0 ? amount : (amount * this.totalShare[token]) / this.totalAmount[token]
  }
...
class LendingPair {
  ...
  toPart(amount) {
    return this.totalBorrowAmount == 0 ? amount : (amount * this.totalBorrowPart) / this.totalBorrowAmount
  }

  toAmount(part) {
    return this.totalBorrowPart == 0 ? part : (part * this.totalBorrowAmount) / this.totalBorrowPart
  }

  addAsset(user, amount) {
    let share = this.bento.deposit(this.asset, user, 'PAIR', amount)
    let fraction = this.totalAssetShare == 0 ? share : (share * totalAssetFraction) / totalAssetShare
