    ...
    function liquidate(address[] calldata users, uint256[] calldata borrowFractions, address to, ISwapper swapper, bool open) public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (!isSolvent(user, open)) {
                ...
                uint256 collateralShare = to18(borrowShare, assetDecimals).mul(liquidationMultiplier) / to18(exchangeRate, assetDecimals); // liqMultiplier
                collateralShare = from18(collateralShare, collateral.decimals());
